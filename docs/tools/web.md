...
- `web_search` — Search the web via Brave Search API (default), Perplexity Sonar (direct or via OpenRouter), Zhipu AI, Gemini with Google Search grounding, or Kimi.
...
| **Kimi**            | AI-synthesized answers, citations              | Requires Kimi/Moonshot API key           | `KIMI_API_KEY` or `MOONSHOT_API_KEY`         |
...
```json5
{
  tools: {
    web: {
      search: {
        provider: "brave", // or "perplexity", "zhipu", "gemini", or "kimi"
      },
    },
  },
}
